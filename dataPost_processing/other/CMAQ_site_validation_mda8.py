...
def calc_daily_mda8_from_hourly(hourly_series):
    """
    将逐小时 O3 序列转换为逐日 MDA8 序列。
    输入长度通常为 daycount*24，输出长度为 daycount。
    规则：
    1. 每天分别计算 17 个 8 小时滑动平均（00-07 到 16-23）
    2. 取当日最大值作为 MDA8
    3. 若某个 8 小时窗口存在缺测，则该窗口记为 NaN
    4. 若某一天全部窗口均无效，则该日 MDA8 为 NaN
    """
    arr = np.asarray(hourly_series, dtype=np.float32)
    n_day = len(arr) // 24
    if n_day == 0:
        return np.asarray([], dtype=np.float32)

    arr = arr[:n_day * 24].reshape(n_day, 24)
    out = []
    for day_vals in arr:
        rolling_means = []
        for i in range(17):
            ...
            if np.isnan(win).any():
                ...
                rolling_means.append(np.mean(win))
        if np.isnan(rolling_means).all():
            out.append(np.nan)
        else:
            out.append(np.nanmax(rolling_means))
...
def CMAQ_site_validation(
    ...
    import netCDF4 as nc
    """
    使用全国公开csv逐小时基本污染物逐小时浓度数据验证CMAQ模拟结果
    ...
    # O3 MDA8 专用处理：
    # 观测端读取 O3_8h 后按日取最大值；模拟端由逐小时 O3 计算逐日 MDA8
    if str(target_substance_obs).upper() == 'O3_8H':
        obs_hourly_like = np.asarray(overall_obs_hourly, dtype=np.float32)
        sim_hourly = np.asarray(overall_sim_hourly, dtype=np.float32)

        n_day_valid = min(len(obs_hourly_like), len(sim_hourly)) // 24
        obs_hourly_like = obs_hourly_like[:n_day_valid * 24]
        sim_hourly = sim_hourly[:n_day_valid * 24]

        obs_daily = obs_hourly_like.reshape(n_day_valid, 24)
        obs_eval = []
        for day_vals in obs_daily:
            if np.isnan(day_vals).all():
                obs_eval.append(np.nan)
            else:
                obs_eval.append(np.nanmax(day_vals))
        obs_eval = np.asarray(obs_eval, dtype=np.float32)

        sim_eval = calc_daily_mda8_from_hourly(sim_hourly)
        x_eval = np.asarray(generate_date_list(start_date, n_day_valid))
        xlabel = 'Date (MDA8 Daily)'
    else:
    ...
    if 'line' in result_pic_types:
        ...
        ax2.set_ylabel(target_substance_obs if target_substance_obs else 'Concentration', fontsize=9)
        ...
        ax2.set_title('Overall Site-Averaged Validation (MDA8 for O3_8h)' if str(target_substance_obs).upper() == 'O3_8H' else 'Overall Site-Averaged Validation', loc='right', fontsize=7)
        fig2.tight_layout()
