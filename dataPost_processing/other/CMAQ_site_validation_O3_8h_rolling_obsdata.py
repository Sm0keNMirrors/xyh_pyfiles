...
def getNearestPos(station_lat, station_lon, XLAT, XLONG):
    """
    得到距离站点最近的经纬度索引值，返回 (row, col)。
    兼容输入网格为 (row, col)、(1, row, col) 等可 squeeze 为二维的情况。
    """
    xlat = np.squeeze(np.asarray(XLAT, dtype=np.float64))
    xlon = np.squeeze(np.asarray(XLONG, dtype=np.float64))
    if xlat.ndim != 2 or xlon.ndim != 2:
        raise ValueError(f'LAT/LON 网格在 squeeze 后必须是二维，当前 LAT={xlat.shape}, LON={xlon.shape}')
    difflat = station_lat - xlat
    difflon = station_lon - xlon
    rad = np.multiply(difflat, difflat) + np.multiply(difflon, difflon)
    flat_idx = int(np.nanargmin(rad))
    row, col = np.unravel_index(flat_idx, rad.shape)
...
def _grid_to_2d(arr):
    ...
    if a.ndim != 2:
        raise ValueError(f'网格数组在 squeeze 后不是二维，shape={a.shape}')
    return a
...
def _safe_grid_value(arr, i, j):
    """兼容可 squeeze 为二维的网格数组取值。"""
    a = _grid_to_2d(arr)
    return a[i, j]
...
def CMAQ_site_validation(
    ...
    var_lon = _grid_to_2d(GRIDCRO2D.variables['LON'][:])
    var_lat = _grid_to_2d(GRIDCRO2D.variables['LAT'][:])
    ...
    CMAQXLAT = _grid_to_2d(GRIDCRO2D.variables['LAT'][:])
    CMAQXLONG = _grid_to_2d(GRIDCRO2D.variables['LON'][:])
    air_csv_data = [pd.read_csv(m) for m in airStation_csv_list]

    obsdata_dir = ''
    if out_dir:
        obsdata_dir = os.path.join(out_dir, 'obsdata')
        if os.path.exists(obsdata_dir) is False:
            os.mkdir(obsdata_dir)
    ...
    for stname, airstation in tqdm(airStation_locations.items(), desc='汇总所有有效站点序列...'):
        nearlat, nearlon = getNearestPos(airstation[1], airstation[0], CMAQXLAT, CMAQXLONG)
        ...
        station_xdate = generate_date_list_withhour(start_date, daycount * 24)
        station_xdate.pop()
        n_export = min(len(station_xdate), len(obs_station), len(sim_station))
        station_xdate = station_xdate[:n_export]
        obs_station_export = np.asarray(obs_station[:n_export], dtype=np.float32)
        sim_station_export = np.asarray(sim_station[:n_export], dtype=np.float32)

        if out_dir:
            import re
            safe_stname = re.sub(r'[\\/:*?"<>|]+', '_', str(stname))
            grid_lon_value = _safe_grid_value(CMAQXLONG, nearlat, nearlon)
            grid_lat_value = _safe_grid_value(CMAQXLAT, nearlat, nearlon)
            station_df = pd.DataFrame({
                'datetime': station_xdate,
                'obs': obs_station_export,
                'sim': sim_station_export,
                'station_name': stname,
                'station_code': station_col,
                'station_lon': airstation[0],
                'station_lat': airstation[1],
                'grid_lon': grid_lon_value,
                'grid_lat': grid_lat_value,
                'city': airstation[3],
            })
            station_df.to_csv(
                os.path.join(obsdata_dir, f'{safe_stname}_{suffix}.csv' if suffix else f'{safe_stname}.csv'),
                index=False,
                encoding='utf-8-sig'
            )

        all_obs_series.append(obs_station_export)
        all_sim_series.append(sim_station_export)
